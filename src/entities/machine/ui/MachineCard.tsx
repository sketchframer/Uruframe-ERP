...
import { Machine, MachineStatus, Job, User } from '@/shared/types';
import {
  Activity,
  AlertTriangle,
  Settings,
...
export const MachineCard = memo<MachineCardProps>(function MachineCard({
  machine,
  projectName,
  activeJob,
  operators,
  ...
  let subStatus = '';
  if (
    machine.type === 'HERRERIA' &&
    machine.status === 'RUNNING' &&
    activeJob?.workflowStages
  ) {
    const activeStage = activeJob.workflowStages.find((s) => !s.isCompleted);
    if (activeStage) subStatus = `ETAPA: ${activeStage.name}`;
  }

  const borderColor = statusColors[machine.status].replace('bg-', 'border-l-');

  return (
    <div
      className={cn(
        'relative p-5 rounded-[1.5rem] border-l-8 transition-all bg-slate-800 shadow-lg hover:shadow-xl hover:scale-[1.01]',
        borderColor,
        className
      )}
    >
      ...
      <div onClick={onClick} className="space-y-3 mt-4 cursor-pointer">
        {machine.status === MachineStatus.RUNNING && (
          <div className="flex items-center text-xs text-slate-300 font-bold">
            ...
            <span className="uppercase tracking-wide">
              {subStatus || `Eficiencia: ${machine.efficiency}%`}
            </span>
          </div>
        )}
          ...
          {operators && operators.length > 0 ? (
            <div className="flex flex-wrap gap-1">
              {operators.map((op) => (
                <span
                  key={op.id}
                  className="bg-slate-700/50 text-slate-300 text-[10px] px-2 py-0.5 rounded-md font-bold"
                >
                  {op.name}
                </span>
              ))}
            ...
            <span className="text-red-500/60 text-[10px] font-black uppercase italic tracking-widest">
              Sin Operadores
            </span>
          )}
        </div>
        ...
        <div className="pt-3 border-t border-slate-700/50 flex flex-col gap-1">
          ...
          <div className="text-xs font-bold text-slate-100 truncate">
            {projectName ?? (
              <span className="text-slate-600 italic">No hay carga de trabajo</span>
            )}
          </div>
        </div>
      </div>
