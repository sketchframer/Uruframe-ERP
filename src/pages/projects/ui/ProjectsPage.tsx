...
export function ProjectsPage() {
  ...
  const handleSaveChanges = async () => {
    if (selectedProjectId && tempProject.id) {
      await projectStore.update(selectedProjectId, tempProject as Partial<Project>);
      setIsEditing(false);
    }
  };
  ...
  const handleAddAccessoryToProject = async () => {
                        ...
                        <div className="flex justify-between items-start mb-4">
                            {isEditing ? (
                                <div className="w-full grid grid-cols-2 gap-4">
                                    ...
                                    <select className="bg-slate-800 border border-slate-600 p-2 rounded text-white" value={tempProject.status} onChange={e => setTempProject({...tempProject, status: e.target.value as Project['status']})}>
                            ...
                            <div className="flex gap-2">
                                {isProjectReadyToDeliver && selectedProject.status !== 'COMPLETED' && selectedProject.status !== 'ARCHIVED' && !isEditing && (
                                    <button
                                        onClick={() => projectStore.update(selectedProject.id, { status: 'COMPLETED' })}
                                ...
                                {selectedProject.status === 'COMPLETED' && !isEditing && (
                                    <button
                                        onClick={() => projectStore.update(selectedProject.id, { status: 'ARCHIVED' })}
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700">
                                <h4 className="text-xs font-bold text-slate-400 uppercase mb-3 flex justify-between">
                                    ...
                                    {wizardStep > 1 && <button onClick={() => setWizardStep(prev => (prev - 1) as 1 | 2 | 3)} className="text-blue-400">Atrás</button>}
                                </h4>
                                    ...
                                    <div className="grid grid-cols-3 gap-3">
                                        {[
                                            ...
                                            <button 
                                                key={opt.type}
                                                onClick={() => { 
                                                    setNewJob({...newJob, machineType: opt.type as Job['machineType']}); 
                                                    setWizardStep(2); 
                                                }}
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {!newJob.isStock && (
                                            <div>
                                                ...
                                                <label className="flex w-full bg-slate-800 border border-slate-600 rounded p-2 cursor-pointer hover:bg-slate-700 items-center justify-center gap-2">
                                ...
                                {projectJobs.map(job => (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <button onClick={(e) => { e.stopPropagation(); jobStore.delete(job.id); }} className="text-slate-600 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity">
                                    ...
                                    return (
                                        <div key={acc.id} className="flex justify-between items-center bg-slate-900 border border-slate-700 p-4 rounded-2xl hover:border-slate-500 group">
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <button onClick={() => projectAccessoryStore.delete(acc.id)} className="text-slate-600 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <Trash2 className="w-4 h-4" />
                                                </button>
                                            </div>
                                        </div>
