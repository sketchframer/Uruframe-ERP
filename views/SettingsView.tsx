...
import { Settings, Users, Database, Bell, Wrench, Trash2, Plus, Power, UserCheck, Briefcase, Monitor, Save, MessageSquare, Send, X, Shield, Cpu, Edit3 } from 'lucide-react';
import { User, Machine, MachineStatus, Role } from '../types';

interface SettingsTarget {
    tab: 'GENERAL' | 'USERS' | 'MACHINES' | 'WORKFORCE' | 'MESSAGES';
    machineId?: string;
}
...
interface SettingsViewProps {
  ...
  targetSettings?: SettingsTarget; 
}

export const SettingsView: React.FC<SettingsViewProps> = ({ users, setUsers, machines, setMachines, onSendMessage, targetSettings }) => {
  const [activeTab, setActiveTab] = useState<'GENERAL' | 'USERS' | 'MACHINES' | 'WORKFORCE' | 'MESSAGES'>('GENERAL');
  ...
  useEffect(() => {
      if(targetSettings) {
          setActiveTab(targetSettings.tab);
      }
  }, [targetSettings]);
  ...
  const handleSaveUser = () => {
    ...
    if (editingUser) {
        setUsers(users.map(u => u.id === editingUser.id ? { ...editingUser, ...userFormData } as User : u));
    } else {
        const user: User = {
            id: `U-${Date.now()}`,
  ...
  const handleSaveMachine = () => {
    ...
    if (editingMachine) {
        setMachines(machines.map(m => m.id === editingMachine.id ? { ...editingMachine, ...machineFormData } as Machine : m));
    } else {
        const machine: Machine = {
            ...machineFormData as Machine,
            id: `M-${Date.now()}`,
  ...
  const handleAssignOperator = (userId: string, machineId: string) => {
    setMachines(machines.map(m => {
        if (m.id === machineId) {
            const currentIds = m.operatorIds || [];
            const newIds = currentIds.includes(userId) 
                ? currentIds.filter(id => id !== userId) 
                : [...currentIds, userId];
            return { ...m, operatorIds: newIds };
        }
        return m;
    }));
  };
  ...
  const handleSend = () => {
    ...
    onSendMessage(msgContent, msgTo);
    setMsgContent('');
        ...
        {[
            ...
            { id: 'MESSAGES', label: 'Mensajería', icon: <MessageSquare size={14}/> },
             ...
             <button 
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-black uppercase transition-all ${activeTab === tab.id ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-700'}`}
            >
                {tab.icon} {tab.label}
            </button>
        ))}
                        ...
                        <div className="bg-slate-800 border border-slate-700 p-8 rounded-[2rem] w-full max-w-lg shadow-2xl">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <select className="bg-slate-900 border border-slate-700 rounded-xl p-4 text-white" value={machineFormData.type} onChange={e => setMachineFormData({...machineFormData, type: e.target.value as any})}>
                                    ...
                                    <select className="bg-slate-900 border border-slate-700 rounded-xl p-4 text-white" value={machineFormData.category} onChange={e => setMachineFormData({...machineFormData, category: e.target.value as any})}>
